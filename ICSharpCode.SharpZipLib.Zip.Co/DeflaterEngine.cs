...
namespace ICSharpCode.SharpZipLib.Zip.Compression
{
	public class DeflaterEngine : DeflaterConstants
	{
		...
		public void FillWindow()
		{
			...
			{
				int num = 8192 - lookahead - strstart;
		...
		private int InsertString()
		{
			...
			short num2 = (prev[strstart & 0xFFF] = head[num]);
		...
		{
			Array.Copy(window, 4096, window, 0, 4096);
			matchStart -= 4096;
			strstart -= 4096;
			blockStart -= 4096;
			for (int i = 0; i < 32768; i++)
			{
				...
				head[i] = (short)((num >= 4096) ? (num - 4096) : 0);
			}
			for (int j = 0; j < 4096; j++)
			{
				int num2 = prev[j] & 0xFFFF;
				prev[j] = (short)((num2 >= 4096) ? (num2 - 4096) : 0);
			}
		}
		...
		private bool FindLongestMatch(int curMatch)
		{
			...
			int num6 = Math.Max(strstart - 3834, 0);
			...
			while ((curMatch = array[curMatch & 0xFFF] & 0xFFFF) > num6 && --num != 0);
		...
		private bool DeflateStored(bool flush, bool finish)
		{
			...
			if (num >= DeflaterConstants.MAX_BLOCK_SIZE || (blockStart < 4096 && num >= 3834) || flush)
			{
				bool flag = finish;
		...
		private bool DeflateFast(bool flush, bool finish)
		{
			...
			while (lookahead >= 262 || flush)
			{
				...
				int num;
				if (lookahead >= 3 && (num = InsertString()) != 0 && strategy != DeflateStrategy.HuffmanOnly && strstart - num <= 3834 && FindLongestMatch(num))
				{
		...
		private bool DeflateSlow(bool flush, bool finish)
		{
			...
			while (lookahead >= 262 || flush)
			{
				...
				if (lookahead >= 3)
				{
					...
					if (strategy != DeflateStrategy.HuffmanOnly && num3 != 0 && strstart - num3 <= 3834 && FindLongestMatch(num3) && matchLen <= 5 && (strategy == DeflateStrategy.Filtered || (matchLen == 3 && strstart - matchStart > 4096)))
					{
						matchLen = 2;
					}
				}
